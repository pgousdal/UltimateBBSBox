"""Preservation-first ABBS family integration for FS-UAE."""
...
class ABBSAmigaIntegration:
    ...
    releases = {
        "3.2-999": ABBSRelease("3.2-999", "3.2", "abbs-amiga-3.2-999-original", "ABBS320_999.lha",
    ...
    def select_release(self, key=None):
        ...
        except KeyError as exc: raise ArtifactRequiredError(f"unknown ABBS release: {key}") from exc

    def _release_for_artifact(self, artifact_id):
        for release in self.releases.values():
            if release.artifact_id == artifact_id: return release
    ...
    def verify_artifacts(self, archive_root, artifact_id):
        ...
        if metadata["artifact"].get("software_family") != "abbs" or metadata["artifact"].get("role") != "preservation_original":
            raise ArtifactRequiredError("ABBS installation requires a classified preservation original")
        if metadata["rights"].get("publish_to_bbs_filebase") or metadata["rights"].get("redistribute_original"):
            raise ArtifactRequiredError("ABBS reference rights must remain deny-by-default")
        if not metadata["rights"].get("install_locally"):
            raise ArtifactRequiredError("rights do not permit local installation")
        if metadata["artifact"]["sha256"] != release.sha256 and metadata["rights"]["status"] != "licensed_private":
            raise ArtifactRequiredError("artifact does not match the selected authentic release")
        return metadata
    ...
    def _extract(source, destination):
        if tarfile.is_tarfile(source):
            with tarfile.open(source) as archive:
                ...
                archive.extractall(destination, filter="data"); return
        lha = shutil.which("lha")
        if not lha: raise InstallError("preparation requires the lha extractor")
        completed = subprocess.run([lha, "x", str(source)], cwd=destination, stdin=subprocess.DEVNULL,
                                   stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, timeout=60, check=False)
        if completed.returncode: raise InstallError(f"lha extractor exited {completed.returncode}")
    ...
    def prepare(self, archive_root, artifact_id, install_root, derived_artifact_id=None):
        ...
        metadata = self.verify_artifacts(root, artifact_id); source = ubb_archive.object_path(root, metadata["artifact"]["sha256"])
        derived_id = derived_artifact_id or f"{artifact_id}-install-tree"
        try: return ubb_archive.verify_one(root, derived_id)
        except (FileNotFoundError, ubb_archive.ArchiveError): pass
        workspace = install_root / "installation" / metadata["artifact"]["sha256"]
        workspace.mkdir(parents=True, exist_ok=True, mode=0o750)
        with tempfile.TemporaryDirectory(prefix="ubb-abbs-") as temp:
            ...
            bundle = pathlib.Path(temp) / "abbs-install-tree.tar"
            ...
            args = self._args(root, self._release_for_artifact(artifact_id))
            args.parent=[artifact_id]; args.file=str(bundle); args.artifact_id=derived_id; args.original_filename="abbs-install-tree.tar"
            args.role="derived_install_media"; args.action="extract LHA into deterministic installation tree"; args.tool="lha/tar"; args.notes="Host-side preparation only; Amiga installation remains assisted."
            args.expected_sha256=args.expected_sha1=args.expected_md5=None
            result = ubb_archive.derive(args)
            if hashlib.sha256(source.read_bytes()).hexdigest() != before: raise InstallError("preservation object changed during preparation")
            return result
    ...
    def configure(self, install_root, evidence=()):
        missing = tuple(x for x in self.manual_evidence if x not in set(evidence))
        return QualificationResult("assisted_amiga_install", QualificationStatus.HUMAN_REQUIRED if missing else QualificationStatus.PASS,
            "Install from installation workspace in AmigaOS, run ConfigNode for serial.device nullmodem mode, then Config BBS; preserve a secret-free golden HDF.", tuple(evidence))
    ...
    def qualify(self, archive_root, artifact_id, install_root, *, evidence=(), **observations):
        ...
        try:
            ...
            results.append(QualificationResult("artifact_integrity", QualificationStatus.PASS, metadata["artifact"]["sha256"]))
            derived=ubb_archive.verify_one(pathlib.Path(archive_root).resolve(), f"{artifact_id}-install-tree")
            lineage=derived.get("preservation", {}).get("lineage", {}).get("parents", [])
            results.append(QualificationResult("derived_lineage", QualificationStatus.PASS if any(x["artifact_id"]==artifact_id for x in lineage) else QualificationStatus.FAIL, "derived installation tree names its immutable parent"))
        except Exception as exc:
            results.append(QualificationResult("artifact_integrity", QualificationStatus.FAIL, str(exc)))
            self._record(install_root, artifact_id, results); return results
        results.append(self.configure(install_root, evidence))
        for check in ("amiga_boot", "abbs_start", "meaningful_readiness", "route_connectivity", "terminal_roundtrip", "login_menu", "clean_stop", "clean_restart", "living_state_survives", "on_demand", "always_on"):
